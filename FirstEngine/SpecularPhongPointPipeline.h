...
public:
	SpecularPhongPointPipeline(GraphicsClass& gfx, std::shared_ptr<ZBuffer> pZb_in, TextureClass& SysBuff) : gfx(gfx), pZb(std::move(pZb_in)), pst(gfx.GetWidth(), gfx.GetHeight()), mWidth(gfx.GetWidth()), mHeight(gfx.GetHeight()), mSysBuff(SysBuff){}
	SpecularPhongPointPipeline(GraphicsClass& gfx) : SpecularPhongPointPipeline(gfx, std::make_shared<ZBuffer>(gfx.GetWidth(), gfx.GetHeight()), gfx.GetTexture()->GetTexture("soft")) {}
	
	void Draw(const IndexedTriangleList& triList) {
...
private:
	...
	void AssembleTriangles(const std::vector<SpecularPhongPointEffect::VSOutput>& vertices, const std::vector<size_t>& indices) {
		...
		const DirectX::XMFLOAT4X4& proj = effect.vs.GetProj();
		DirectX::XMVECTOR eyepos = XMVectorSet(proj(3, 0), proj(3, 1), proj(3, 2), 1.0f);

		// assemble triangles in the stream and process
		for (size_t i = 0, end = indices.size() / 3; i < end; i++) {

			// determine triangle vertices via indexing
			const SpecularPhongPointEffect::VSOutput& v0 = vertices[indices[i * 3]];
			const SpecularPhongPointEffect::VSOutput& v1 = vertices[indices[i * 3 + 1]];
			const SpecularPhongPointEffect::VSOutput& v2 = vertices[indices[i * 3 + 2]];
	...
	void ProcessTriangle(const SpecularPhongPointEffect::VSOutput& v0, const SpecularPhongPointEffect::VSOutput& v1, const SpecularPhongPointEffect::VSOutput& v2, size_t triangle_index) {
		// generate triangle from 3 vertices using gs
		// and send to clipper
		ClipCullTriangle(effect.gs(v0, v1, v2, triangle_index));
	}

	void ClipCullTriangle(const Triangle<SpecularPhongPointEffect::VSOutput>& t) {
		// cull tests
		...
		// clipping routines
		const auto Clip1 = [this](const SpecularPhongPointEffect::VSOutput& v0, const SpecularPhongPointEffect::VSOutput& v1, const SpecularPhongPointEffect::VSOutput& v2) {
			...
			const SpecularPhongPointEffect::VSOutput v0a = v0 + (v1 - v0) * alphaA;
			const SpecularPhongPointEffect::VSOutput v0b = v0 + (v2 - v0) * alphaB;

			// draw triangles
			PostProcessTriangleVertices(Triangle<SpecularPhongPointEffect::VSOutput>{v0a, v1, v2});
			PostProcessTriangleVertices(Triangle<SpecularPhongPointEffect::VSOutput>{v0b, v0a, v2});
		};
		const auto Clip2 = [this](const SpecularPhongPointEffect::VSOutput& v0, const SpecularPhongPointEffect::VSOutput& v1, const SpecularPhongPointEffect::VSOutput& v2) {
			// calculate alpha values for getting adjusted vertices
			const float alpha0 = (-v0.pos.z) / (v2.pos.z - v0.pos.z);
			const float alpha1 = (-v1.pos.z) / (v2.pos.z - v1.pos.z);

			// interpolate to get v0a and v0b
			const SpecularPhongPointEffect::VSOutput v0a = v0 + (v2 - v0) * alpha0;
			const SpecularPhongPointEffect::VSOutput v1b = v1 + (v2 - v1) * alpha1;

			// draw triangles
			PostProcessTriangleVertices(Triangle<SpecularPhongPointEffect::VSOutput>{ v0a, v1b, v2 });
		};
	...
	void PostProcessTriangleVertices(const Triangle<SpecularPhongPointEffect::VSOutput>& triangle) {
	...
	void DrawTriangle(const Triangle<SpecularPhongPointEffect::VSOutput>& triangle) {

		// using pointers so we can swap (for sorting purposes)
		const SpecularPhongPointEffect::VSOutput* pv0 = &triangle.v0;
		const SpecularPhongPointEffect::VSOutput* pv1 = &triangle.v1;
		const SpecularPhongPointEffect::VSOutput* pv2 = &triangle.v2;
		...
		// general triangle
			...
			const SpecularPhongPointEffect::VSOutput vi = *pv0 + (*pv2 - *pv0) * alphaSplit;
	...
	void DrawFlatTopTriangle(const SpecularPhongPointEffect::VSOutput& it0, const SpecularPhongPointEffect::VSOutput& it1, const SpecularPhongPointEffect::VSOutput& it2) {
	...
	void DrawFlatBottomTriangle(const SpecularPhongPointEffect::VSOutput& it0, const SpecularPhongPointEffect::VSOutput& it1, const SpecularPhongPointEffect::VSOutput& it2) {
		// calulcate dVertex / dy
		// change in interpolant for every 1 change in y
		const float delta_y = it2.pos.y - it0.pos.y;
	...
	void DrawFlatTriangle(const SpecularPhongPointEffect::VSOutput& it0, const SpecularPhongPointEffect::VSOutput& it1, const SpecularPhongPointEffect::VSOutput& it2, const SpecularPhongPointEffect::VSOutput& dv0, const SpecularPhongPointEffect::VSOutput& dv1, SpecularPhongPointEffect::VSOutput itEdge1) {
		// create edge interpolant for left edge (always v0)
		auto itEdge0 = it0;

		// calculate start and end scanlines
		const int yStart = std::max((int)std::ceil(it0.pos.y - 0.5f), 0);
		const int yEnd = std::min((int)std::ceil(it2.pos.y - 0.5f), (int)mHeight - 1); // the scanline AFTER the last line drawn
		...
		for (int y = yStart; y < yEnd; y++, itEdge0 += dv0, itEdge1 += dv1) {
			...
			const int xEnd = std::min((int)std::ceil(itEdge1.pos.x - 0.5f), (int)mWidth - 1); // the pixel AFTER the last pixel drawn
