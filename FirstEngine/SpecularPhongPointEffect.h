...
public:
	...
	public:
		...
		void BindWorldView(const DirectX::XMFLOAT4X4& transformation_in) {
		...
		void BindProjection(const DirectX::XMFLOAT4X4& transformation_in) {
	...
	public:
		...
		Triangle<VSOutput> operator()(const VSOutput& in0, const VSOutput& in1, const VSOutput& in2, size_t triangle_index) const {
			return { in0, in1, in2 };
		}
	};
	...
	public:
		...
		ColorIntegers Shade(const VSOutput& in, const DirectX::XMFLOAT3& material_color) const {
			...
			DirectX::XMVECTOR d = DirectX::XMVectorScale(light_diffuseXM, attenuation * std::max(0.0f, DirectX::XMVectorGetX(DirectX::XMVector3Dot(surf_normXM,  dir))));
			...
			DirectX::XMVECTOR s = DirectX::XMVectorScale(light_diffuseXM, SpecularParams::specular_intensity * std::pow(std::max(0.0f, DirectX::XMVectorGetX(DirectX::XMVector3Dot(DirectX::XMVectorScale(rn, -1.0f), DirectX::XMVector3Normalize(worldPosXM)))), SpecularParams::specular_power));
		...
		void SetDiffuseLight(const DirectX::XMFLOAT3& c) {
			light_diffuse = c;
		}

		void SetAmbientLight(const DirectX::XMFLOAT3& c) {
			light_ambient = c;
		}

		void SetLightPosition(const DirectX::XMFLOAT3& pos_in) {
			light_pos = pos_in;
		}

		ColorIntegers operator()(const VSOutput& in) const {
			...
			const auto material_color_t = DirectX::XMFLOAT3(
				color.GetR() / 255.0f,
				color.GetG() / 255.0f,
				color.GetB() / 255.0f
			);
		...
		void BindTexture(const TextureClass& tex) {
			pTex = &tex;
	...
	private:
		...
		const TextureClass* pTex = nullptr;
